...
class EditScoreModal extends React.Component<Props> {
    renderTeamScore = (team: I.Team | undefined, score: number) => {
        ...
        return <div key={team._id} className="team-score-container">
            ...
            <div className="winner-button-container">
                <Button className={`lightblue-btn round-btn ${isWinner ? 'unset':''}`} onClick={setWinner(isWinner ? undefined : team._id)}>{ isWinner ? 'Unset win':'Set win'}</Button>
            </div>
        </div>
    }
    render() {
        ...
        return (
            <Modal isOpen={isOpen} toggle={toggle} className={'veto_modal'}>
                <ModalHeader className="bordered" toggle={toggle}>{veto.mapName.toUpperCase()}</ModalHeader>
                <ModalBody>
