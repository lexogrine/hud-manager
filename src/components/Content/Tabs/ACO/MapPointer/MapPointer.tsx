...
const MapPointer = ({ config, file, onPointAdd, areas, onClickArea }: IProps) => {
    ...
    const generatePolygon = (area: MapAreaConfig) => {
        const points = area.polygonCorners.map(point => gameUnitsToRadar(point[0], point[1])).map(coord => `${coord.x}px ${coord.y}px`);
    ...
    return (
        <div className="aco_picker">
            {areas.map(area => <React.Fragment key={area.name}>
                <div
                    onClick={() => onClickArea(area)}
                    className="aco_area"
                    style={{ clipPath: generatePolygon(area), backgroundColor: textToRGB(area.name) }}
                />
                {area.polygonCorners.map(corner => <div key={`${corner[0]}${corner[1]}`} className="aco_dot" style={{backgroundColor: textToRGB(area.name), left: `${gameUnitsToRadar(corner[0], corner[1]).x - 2.5}px`, top: `${gameUnitsToRadar(corner[0], corner[1]).y - 2.5}px`}} />)}
            </React.Fragment>)}
