...
const ACO = ({ toggle }: IProps) => {
    ...
    const saveArea = () => {
        ...
        api.aco.set(currentACO).then(loadACOs).then(() => { setModalOpen(false); setNewArea(null) });
    }
    ...
    const saveConfig = (cfg: string) => {
        ...
        api.aco.set(currentACO).then(loadACOs).then(() => setConfigOpen(false));
    }
    ...
    return (
        <React.Fragment>
            <SaveAreaModal
                ...
                close={() => { setModalOpen(false); setNewArea(null); }}
            ...
            <div className={`tab-content-container full-scroll`}>
                <Row className="padded">
                    <div style={{ width: "512px", marginRight:'25px' }}>
                        ...
                        <MapPointer config={config} file={activeMapConfig.file} onClickArea={onClickArea} onPointAdd={onPointAdd} areas={newArea ? [{ name: "newarea", polygonCorners: newArea, configs: [], priority: 0 }] : areas} />
                        <div className="btn-container">
                            {!newArea ? (
                                <button
                                    type="button"
                                    className="round-btn run-game btn btn-secondary add_aco_config"
                                    onClick={() => { setNewArea([]); setActiveConfig(null); }}>Add area</button>
                            ) : (
                                    <>
                                        <button
                                            type="button"
                                            className="round-btn run-game btn btn-secondary add_aco_config"
                                            onClick={addNewArea}>Save area
                                    </button>
                                        <button
                                            type="button"
                                            className="round-btn run-game btn btn-secondary add_aco_config"
                                            onClick={() => setNewArea(null)}>Clear area
                                    </button>
                                    </>
                                )}
                        </div>
                    ...
                    {activeConfig ? (
                        <div style={{ flex: 1, minWidth: '386px' }}>
                            ...
                            {activeConfig.configs.map(config => <div
                                key={config}
                                className="aco_area_config"
                            >
                                {config.length > 30 ? config.substring(0, 30) + '...' : config}
									<img src={trash} className="action" alt="Delete Config" onClick={() => removeConfigFromArea(config)} />
                            </div>)}
                            <button type="button" className="round-btn run-game btn btn-secondary add_aco_config" onClick={() => setConfigOpen(true)}>Add config</button>
                            <button type="button" className="round-btn run-game btn btn-secondary add_aco_config" onClick={removeCurrentArea}>Remove area</button>
                        </div>
                    ) : null}
