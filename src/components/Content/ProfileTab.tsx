							...
							cxt.workspaces.length > 1 ? (
								<div className="workspace-select">
									...
									<LabeledInput
										...
										onChange={e => setToNewWorkspace(Number(e.target.value), cxt).then(() => { cxt.reload() })}
									>
										{cxt.workspaces.map(workspace => (
											<option disabled={!isWorkspaceAvailable(workspace.id, cxt)} value={workspace.id} key={`edit--${workspace.name}--${workspace.id}`}>
												{workspace.name}
											</option>
										))}
									</LabeledInput>
