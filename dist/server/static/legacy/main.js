...
$(document).ready(function () {
    const methods = {
        ...
        getGrenades: function () {
            ...
            if (weapons === false)
                return false;
            for (var k in weapons) {
                if (weapons[k].type == "Grenade")
                    grenades.push(weapons[k]);
            }
            return grenades;
        },
        getStats: function () {
    ...
    const functions = {
        getTeamOne: function () {
            if (!this.info.teams || !this.info.teams.team_1)
                return false;
            return this.getTeam(this.info.teams.team_1.team);
        },
        getTeamTwo: function () {
            if (!this.info.teams || !this.info.teams.team_2)
                return false;
        ...
        getObserved: function () {
            if (!this.info.player || this.info.player.steamid == 1)
                return false;
        ...
        getPlayer: function (slot) {
            ...
            for (var k in players) {
                if (players[k].observer_slot === slot)
                    return players[k];
            }
            return false;
        },
        phase: function () {
            if (!this.info.phase_countdowns)
                return false;
