...
exports.generateAfxLutFile = void 0;
        ...
        for (const iG of INDEXES.GREEN) {
            for (const iB of INDEXES.BLUE) {
                for (const iA of INDEXES.ALPHA) {
                    const currentReferenceAddress = [iR * 255, iG * 255, iB * 255, iA * 255];
                    let bestColor;
...
const generateAfxLutFile = (outputFile, ctXray, tXray) => {
    const ctXrayTransparent = [...ctXray, 0];
    const ctXrayVisible = [...ctXray, 255];
    const tXrayTransparent = [...tXray, 0];
    const tXrayVisible = [...tXray, 255];
    ...
    const addToBuffer = (buf) => {
        bufferContext.buffer = buffer_1.Buffer.concat([bufferContext.buffer, buf]);
    };
    const bufferContext = {
        buffer: buffer_1.Buffer.alloc(11),
        writeInt32LE: (value, offset) => {
            const temporaryBuffer = buffer_1.Buffer.alloc(4);
        ...
        writeUInt8: (value, offset) => {
            const temporaryBuffer = buffer_1.Buffer.alloc(1);
    ...
    fs_1.default.writeFileSync(outputFile, bufferContext.buffer);
};
exports.generateAfxLutFile = generateAfxLutFile;
