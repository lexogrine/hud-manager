...
async function importPlayers(players: I.Player[]){
    return new Promise<I.Player[]>((res, rej) => {
        ...
        playersDb.remove({ $or: playerIdList }, { multi: true }, (err, n) => {
            if(err) return res([]);
...
async function importTeams(teams: I.Team[]){
    return new Promise<I.Team[]>((res, rej) => {
        ...
        teamsDb.remove({ $or: teamIdList }, { multi: true }, (err, n) => {
            if(err) return res([]);
...
export const checkForConflicts: express.RequestHandler = async (req,res) => {
    ...
    try {
        const result = await Promise.all([players.getPlayersList({ $or: playerIdList }), teams.getTeamsList({ $or: teamIdList })]);
        return res.json({
