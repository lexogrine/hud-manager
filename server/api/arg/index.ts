...
export const argSocket: { socket: SimpleWebSocket | null, id: string | null, delay: number } = { delay: 7, socket: null, id: null };
...
export const sendKillsToARG = (last: CSGO, csgo: CSGO) => {
	...
	const playerKills: KillStatistics[] = csgo.players.map(player => ({ steamid: player.steamid, kills: player.stats.kills, health: player.state.health, name: player.name }));
	const oldPlayerKills: KillStatistics[] = last.players.map(player => ({ steamid: player.steamid, kills: player.stats.kills, health: player.state.health, name: player.name }));
