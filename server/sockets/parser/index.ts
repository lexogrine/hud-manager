...
export class Encoder {
	...
	private encodeAsString(obj: Packet) {
		...
		// json data
		if (null != obj.data) {
			str += JSON.stringify(obj.data, (key, value) => (typeof value === 'bigint' ? value.toString() : value));
		}
...
export class Decoder extends Emitter<{}, {}, DecoderReservedEvents> {
	private reconstructor?: BinaryReconstructor | null;
	...
	private decodeString(str: string): Packet {
		...
		if (p.type === PacketType.BINARY_EVENT || p.type === PacketType.BINARY_ACK) {
			...
			const buf = str.substring(start, i) as string | number;
		...
		// look up id
		const next = str.charAt(i + 1) as string | number;
		if ('' !== next && Number(next) == next) {
			const start = i + 1;
			while (++i) {
				const c = str.charAt(i) as string | number;
